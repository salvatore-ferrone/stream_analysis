...
def main():
    ...
    for MASS_INDEX in range(N_GRID):
        for RADIUS_INDEX in range(N_GRID):
            ...
            plot_title = "{:s} {:s} Mass: {:d} Msun, Radius: {:d} pc".format(GCname, montecarlokey, int(np.floor(MASS)), int(np.ceil(1000*RC)))
            figure_name = "./frames/{:s}-{:s}-mass-{:d}-radius-{:d}.png".format(GCname, montecarlokey, int(np.floor(MASS)), int(np.ceil(1000*RC)))
