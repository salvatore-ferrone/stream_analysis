...
%DIF DEL ../prereview_main_no_appendixE.tex   Tue May 27 17:35:33 2025
%DIF ADD aa53923-25.tex                       Tue May 27 16:21:51 2025
%DIF < \documentclass{aa}
%DIF < 
%DIF < 
%DIF < % \usepackage[draft]{graphicx}
%DIF -------
...
%DIF < %\title{Globular clusters fly-bys and gap creations in stellar streams.}
%DIF -------
...
\author{Salvatore Ferrone
       \DIFdelbegin %DIFDELCMD < \inst{1,2}
%DIFDELCMD <          %%%
\DIFdelend \DIFaddbegin \inst{1,2,3}
         \DIFaddend \and
       Marco Montuori\DIFdelbegin %DIFDELCMD < \inst{2}
%DIFDELCMD <        %%%
\DIFdelend \DIFaddbegin \inst{3}
       \DIFaddend \and
       Paola Di Matteo\inst{1}
       \and
       Alessandra Mastrobuono-Battisti \DIFdelbegin %DIFDELCMD < \inst{3}
%DIFDELCMD <        %%%
\DIFdelend \DIFaddbegin \inst{4}
       \DIFaddend \and 
       Rodrigo Ibata \DIFdelbegin %DIFDELCMD < \inst{4}
%DIFDELCMD <        %%%
\DIFdelend \DIFaddbegin \inst{5}
       \DIFaddend \and 
       Paolo Bianchini \DIFdelbegin %DIFDELCMD < \inst{4}
%DIFDELCMD <        %%%
\DIFdelend \DIFaddbegin \inst{5}
       \DIFaddend \and
       Sergey Khoperskov \DIFdelbegin %DIFDELCMD < \inst{5}
%DIFDELCMD <        %%%
\DIFdelend \DIFaddbegin \inst{6}
       \DIFaddend \and
       Nicolas Leclerc \DIFdelbegin %DIFDELCMD < \inst{6}
%DIFDELCMD <        %%%
\DIFdelend \DIFaddbegin \inst{7}
       \DIFaddend \and 
       Clement Hottier \DIFdelbegin %DIFDELCMD < \inst{6}
%DIFDELCMD <        %%%
\DIFdelend \DIFaddbegin \inst{7}
       \DIFaddend \and 
       Eliot Stein  \DIFdelbegin %DIFDELCMD < \inst{7}
%DIFDELCMD <        %%%
\DIFdelend \DIFaddbegin \inst{8}
       \DIFaddend \and
       David Valls-Gabaud \DIFdelbegin %DIFDELCMD < \inst{8}
%DIFDELCMD <        %%%
\DIFdelend \DIFaddbegin \inst{9}
       \DIFaddend \and
       Owain N. Snaith \DIFdelbegin %DIFDELCMD < \inst{9}
%DIFDELCMD <        %%%
\DIFdelend \DIFaddbegin \inst{10}
       \DIFaddend \and
       Misha Haywood \inst{1}
       }

\institute{
    LIRA, Observatoire de Paris, Université PSL, Sorbonne Université, Université Paris Cité, CY Cergy Paris Université, CNRS,  \DIFdelbegin \DIFdel{75014 Paris}\DIFdelend \DIFaddbegin \DIFadd{92190 Meudon}\DIFaddend , France\\
  \email{salvatore.ferrone@obspm.fr}
  \and
    \DIFdelbegin \DIFdel{Dipartimento di Fisica, Universit\`a di Roma }\DIFdelend \DIFaddbegin \DIFadd{Department of Physics, University of Rome }\DIFaddend ``La Sapienza'', \DIFdelbegin \DIFdel{Piazza Aldo Moro }\DIFdelend \DIFaddbegin \DIFadd{Piazzale Aldo Moro 5, 00185 Rome, Italy.
  }\and 
    \DIFadd{Institute for Complex Systems CNR, Piazzale Aldo Moro 2, 00185 Rome, Italy
  }\DIFaddend \and
...
%DIFDELCMD <   
%DIFDELCMD <   %%%
\DIFdelend Beyond the global visible and dark mass distribution streams\DIFdelbegin \DIFdel{can be used also }\DIFdelend \DIFaddbegin \DIFadd{, they can also be used }\DIFaddend to infer the granularity of the dark matter, that is\DIFaddbegin \DIFadd{, }\DIFaddend the mass and density of the sub-halos populating our \DIFdelbegin \DIFdel{Galaxy}\DIFdelend \DIFaddbegin \DIFadd{galaxy}\DIFaddend . According to simulations by \citet{2008MNRAS.391.1685S}, \DIFaddbegin \DIFadd{the }\DIFaddend $\Lambda$~Cold Dark Matter ($\Lambda$CDM) \DIFaddbegin \DIFadd{model }\DIFaddend predicts that galaxies grow hierarchically, with dark matter clumps forming at a wide range of masses and sizes. These clumps, or \DIFdelbegin \textit{\DIFdel{sub-halos}}%DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{sub-halos}\DIFaddend , are predicted to follow a mass distribution with a power-law slope slightly shallower than -2.0. \DIFdelbegin \DIFdel{To date, }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
...
\citep{2016ApJ...828L..10H, 2021MNRAS.507.1999H, 2021JCAP...10..043B, 2024arXiv240402953H, 2024arXiv241021174N}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{2016ApJ...828L..10H, 2021MNRAS.507.1999H, 2021JCAP...10..043B, 2024arXiv240402953H, 2025ApJ...983...68N}}\hskip0pt%DIFAUXCMD
}\DIFaddend , or searched for their traces in observed streams \citep{2016MNRAS.460.2711T, 2017MNRAS.470...60E, 2020ApJ...889...70B, 2020ApJ...892L..37B}.

  In contrast, a \DIFdelbegin \DIFdel{more }\DIFdelend limited number of works have \DIFdelbegin \DIFdel{been interested in understanding }\DIFdelend \DIFaddbegin \DIFadd{explored }\DIFaddend whether other structures, \DIFdelbegin \DIFdel{e.g. }\DIFdelend \DIFaddbegin \DIFadd{such as those }\DIFaddend from baryonic matter, can cause variations in the density of streams and gaps that can be \DIFdelbegin \DIFdel{assimilated and }\DIFdelend confused with those produced by dark matter sub-halos.  Among these works, it is worth mentioning the results of \citet{2017NatAs...1..633P}\DIFdelbegin \DIFdel{that suggests how }\DIFdelend \DIFaddbegin \DIFadd{, which suggest that }\DIFaddend the presence of the bar at the center of the Galaxy can perturb the characteristics of a stream\DIFaddbegin \DIFadd{, }\DIFaddend such as Palomar~5\DIFaddbegin \DIFadd{, }\DIFaddend and generate gaps along its tail. That the Galactic bar could have an influence on \DIFdelbegin \DIFdel{streams }\DIFdelend \DIFaddbegin \DIFadd{stream }\DIFaddend morphology was also discussed by \citet{2016MNRAS.460..497H} and \citet{2016ApJ...824..104P}, in the case of the Ophiuchus stream \citep{2014MNRAS.443L..84B}. Besides the Galactic bar, \DIFdelbegin \DIFdel{also }\DIFdelend giant molecular clouds can \DIFaddbegin \DIFadd{also }\DIFaddend produce gaps in stellar streams, as shown by \citet{2016MNRAS.463L..17A}. All these works thus \DIFdelbegin \DIFdel{seem to }\DIFdelend indicate that baryonic structures can play an important role in tail morphology. In this context,  an extensive numerical study, specifically focused on modeling the tails of Palomar~5, under the influence of the Galactic bar, spiral arms, giant molecular clouds\DIFaddbegin \DIFadd{, }\DIFaddend and globular clusters, has been realized by \citet{2019MNRAS.484.2009B}, who concluded that both the influence of the bar and that of the giant molecular clouds can leave imprints in Palomar~5 tidal tails similar to those left by dark matter sub-halos\DIFdelbegin \DIFdel{, while }\DIFdelend \DIFaddbegin \DIFadd{. In contrast, they found }\DIFaddend the effect of globular clusters \DIFdelbegin \DIFdel{was found }\DIFdelend to be negligible. 
...
; we then summarize the method used to model the mass loss from the cluster (Sect.~\ref{sec:mass_loss}) and finally discuss the quality of the numerical integration (Sect.~\ref{num_quality}). 
  ...
  \subsection{Numerical \DIFdelbegin \DIFdel{Methodology}\DIFdelend \DIFaddbegin \DIFadd{methodology}\DIFaddend }\DIFaddbegin \label{numerical_meth}
    ...
    \DIFdelbegin \subsubsection*{\texttt{\DIFdel{Full}} %DIFAUXCMD
\DIFdel{simulations}}
    %DIFAUXCMD
\DIFdelend \DIFaddbegin \subsubsection{ \texttt{\DIFadd{Full}} \DIFadd{simulations}}
    \DIFaddend There is a primary methodological departure from \citet{2023A&A...673A..44F}.  In that work, globular clusters evolved under the gravitational effect of the Galaxy \DIFdelbegin \DIFdel{only, while now we include also }\DIFdelend \DIFaddbegin \DIFadd{alone. In contrast, now we also consider }\DIFaddend the effect of all \DIFdelbegin \DIFdel{the }\DIFdelend other Galactic globular clusters \DIFdelbegin \DIFdel{, }\DIFdelend by taking into account the direct $N$-body interactions \DIFdelbegin \DIFdel{among them. To quantify these interactions}\DIFdelend \DIFaddbegin \DIFadd{between them. First}\DIFaddend , all clusters are represented by Plummer spheres, each with \DIFaddbegin \DIFadd{its own }\DIFaddend mass and half-mass \DIFdelbegin \DIFdel{radii }\DIFdelend \DIFaddbegin \DIFadd{radius }\DIFaddend as reported in the Baumgardt catalog \citep{2021MNRAS.505.5957B}. For the remainder of this paper, \DIFdelbegin \DIFdel{we refer to these simulations , where the globular clusters interactionsare taken into account as }\texttt{\DIFdel{full}} %DIFAUXCMD
\DIFdel{simulations}\DIFdelend \DIFaddbegin \DIFadd{the }\texttt{\DIFadd{full}} \DIFadd{simulations consider the gravitational forces from the globular cluster interactions}\DIFaddend . \\
    For these simulations, we proceed \DIFdelbegin \DIFdel{into }\DIFdelend \DIFaddbegin \DIFadd{in }\DIFaddend two steps:
      \begin{enumerate}
          ...
          \noindent where $\vec{r}$ indicates the galactocentric position vector, the index $i$ indicates the globular cluster of interest\DIFdelbegin \DIFdel{with $R_i,z_i$ being its galactocentric positions in cylindrical coordinates}\DIFdelend ; the index $j$ indicates the other globular clusters that are summed over. $N_{GC}$ is the total number of globular clusters\DIFaddbegin \DIFadd{, }\DIFaddend which in this study is 165, $m_j$ is the mass of the j-th cluster in the sample, $b_j$ is its Plummer scale radius, and $\vec{r_j}$ is its galactocentric position. $\Phi$ represents the same Galactic smooth potential that we discussed previously \citep[][Model~II, in the present case]{2017A&A...598A..66P}. Note that the masses and sizes of the globular clusters are kept constant in these simulations and \DIFaddbegin \DIFadd{are }\DIFaddend not allowed to vary with time\DIFdelbegin \DIFdel{. This equally implies }\DIFdelend \DIFaddbegin \DIFadd{, which means }\DIFaddend that we do not \DIFdelbegin \DIFdel{take into account the internal evolutionof the globular clusters in these models. We }\DIFdelend \DIFaddbegin \DIFadd{consider their internal evolution. In sec.~\ref{sec:discussion}, we }\DIFaddend discuss the implications of the modeling limitations\DIFdelbegin \DIFdel{in the Discussion}\DIFdelend .

        \item Once \DIFaddbegin \DIFadd{we find }\DIFaddend the positions and velocities of the entire globular cluster\DIFdelbegin \DIFdel{system 5~Gyr ago are found}\DIFdelend , we sample Palomar~5 with 100,000 particles from a Plummer distribution\DIFaddbegin \DIFadd{, }\DIFaddend taking the mass and half-mass radius from the Baumgardt catalog\DIFaddbegin \DIFadd{: $1.3\times10^{4}~\textrm{M}_\odot$ and $27.6~\textrm{pc}$}\DIFaddend . We then integrate the evolution of these particles forward in time to the present day, \DIFdelbegin \DIFdel{by }\DIFdelend taking into account that each particle feels the gravitational potential of the Galaxy, \DIFdelbegin \DIFdel{that of }\DIFdelend its host cluster, and that of all the other clusters in the Galaxy. Note that we do not \DIFdelbegin \DIFdel{take into account }\DIFdelend \DIFaddbegin \DIFadd{account for }\DIFaddend self-gravity among particles. The particles experience the gravitational field yet do not contribute to it, \DIFdelbegin \DIFdel{which is }\DIFdelend a common assumption \DIFdelbegin \DIFdel{made in Galactic dynamicssince }\DIFdelend \DIFaddbegin \DIFadd{in galactic dynamics, as }\DIFaddend the mass of an individual star is negligible compared to the mass of the larger dynamical system. The equation of motion of a generic particle among the 100,000 \DIFdelbegin \DIFdel{which }\DIFdelend \DIFaddbegin \DIFadd{that }\DIFaddend populate Palomar~5 is thus: 
        \begin{equation}
    ...
    \begin{table}[h]
      \centering
      \DIFaddbeginFL \caption{\DIFaddFL{The parameters determining the data volume}}
      \label{tab:data_volume}
      \DIFaddendFL \begin{tabular}{|c|c|c|c|}
          \hline
...
{%DIFAUXCMD
\DIFdelFL{The parameters determining the data volume of this experiment. where $N_p$  is the number of particles, $N_{\textrm{ts}}$ is the number of time-steps saved, $N_{\textrm{phase}}$ is the number of phase space coordinates, and $N_{\textrm{sampling}}$ is the number of Monte-Carlo samplings of the initial conditions.}}
      %DIFAUXCMD
%DIFDELCMD < \label{tab:data_volume}
%DIFDELCMD <     %%%
\DIFdelendFL \DIFaddbeginFL \tablefoot{$N_p$  is the number of particles, $N_{\textrm{ts}}$ is the number of time-steps saved, $N_{\textrm{phase}}$ is the number of phase space coordinates, and $N_{\textrm{sampling}}$ is the number of Monte-Carlo samplings of the initial conditions.}
    \DIFaddendFL \end{table}

  \DIFdelbegin \subsubsection*{\texttt{\DIFdel{Reference}} %DIFAUXCMD
\DIFdel{simulations}}
%DIFAUXCMD
\DIFdelend \DIFaddbegin \subsubsection{ \texttt{\DIFadd{Reference}} \DIFadd{simulations}}
\DIFaddend 
...
\DIFaddend The \DIFdelbegin \DIFdel{integration was conducted using a leapfrog integrator, chosen for }\DIFdelend \DIFaddbegin \DIFadd{assumption in point 2 is a direct consequence of the approach described in point 1. Starting from a cluster with a mass and size similar to the current ones can lead to streams with lower velocity dispersions than those we would obtain if we had used a self-consistent approach. In a future article, we will report on the study of gap survival times depending on the masses and sizes of progenitor clusters (Ferrone et al, in prep). We note, however, that simplifications of this kind are not uncommon in literature. \mbox{%DIFAUXCMD
...
\section{Results}
  ...
  \subsection{Overview}
    ...
    \begin{figure*}
      ...
      \DIFdelbeginFL %DIFDELCMD < \caption{%%%
\DIFdelFL{Density maps of Palomar~5's stream in the tail coordinate system. The color scale represents normalized particle counts (total: 100,000). The top panel shows the }\texttt{\DIFdelFL{full}} %DIFAUXCMD
\DIFdelFL{simulation with three gaps on the stream's right-hand side. The next three panels depict simulations with identical initial conditions but exclude the gravitational influence of all clusters except those forming a given gap. The }\texttt{\DIFdelFL{Reference}} %DIFAUXCMD
\DIFdelFL{simulation omits the influence from other globular clusters. The bottom panel compares Palomar~5's observed stream length to the }\texttt{\DIFdelFL{Reference}} %DIFAUXCMD
\DIFdelFL{simulation, using the same Monte Carlo realization as Fig.~\ref{fig:stream_on_sky} and }\texttt{\DIFdelFL{Sampling~009}} %DIFAUXCMD
\DIFdelFL{of Fig.~gallery2.
      }\DIFdelendFL \DIFaddbeginFL \caption{\DIFaddFL{Density maps of Palomar~5's stream in the tail coordinate system, where $x\prime$ is the integrated arclength along the cluster's orbit and $y\prime$ is the distance within the orbital plane. The color scale represents normalized particle counts (total: 100,000). The top panel shows the }\texttt{\DIFaddFL{full}} \DIFaddFL{simulation with three gaps on the stream's right-hand side. The next three panels depict simulations with identical initial conditions but exclude the gravitational influence of all clusters except those forming a given gap. The }\texttt{\DIFaddFL{Reference}} \DIFaddFL{simulation omits the influence from other globular clusters. The bottom panel compares Palomar~5's observed stream length to the }\texttt{\DIFaddFL{Reference}} \DIFaddFL{simulation, using the same Monte Carlo realization as Fig.~\ref{fig:stream_on_sky} and }\texttt{\DIFaddFL{Sampling~009}} \DIFaddFL{of Fig.~\ref{fig:gallery2}.}}
...
\DIFdelend The presence of the other globular clusters \DIFdelbegin \DIFdel{has a clear effect on }\DIFdelend \DIFaddbegin \DIFadd{affects }\DIFaddend the properties of \DIFaddbegin \DIFadd{the }\DIFaddend Palomar~5 stream. \DIFdelbegin \DIFdel{A particularly clear example of this effect is shown in }\DIFdelend Fig.~\ref{fig:stream_on_sky} \DIFaddbegin \DIFadd{presents an obvious example of this effect, }\DIFaddend which we selected for its prominent gaps. Two of these gaps are \DIFdelbegin \DIFdel{clearly }\DIFdelend visible in Galactic coordinates and become even more apparent when marginalizing over latitude to reconstruct the 1D density profile of the stream \DIFdelbegin \DIFdel{, }\DIFdelend as a function of longitude.     
    ...
    Regarding the shape of the density distribution, the central peak corresponds to the still-intact globular cluster whose stars have not yet escaped. The \DIFdelbegin \DIFdel{density peaks in the stream }\DIFdelend \DIFaddbegin \DIFadd{stream's density peaks }\DIFaddend are of the same order as the cluster itself\DIFdelbegin \DIFdel{. This }\DIFdelend \DIFaddbegin \DIFadd{, which }\DIFaddend is inconsistent with reality\DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{; }\DIFaddend the cluster's peak density should be higher than \DIFdelbegin \DIFdel{the stream's peak density}\DIFdelend \DIFaddbegin \DIFadd{that of the stream}\DIFaddend . Of course, this discrepancy is a result of our modeling choices. \DIFdelbegin \DIFdel{We use the present day }\DIFdelend \DIFaddbegin \DIFadd{Since we use the present-day }\DIFaddend mass and radius for Palomar~5 for the whole \DIFdelbegin \DIFdel{duration of the simulation. Consequently the system is modeled as being }\DIFdelend \DIFaddbegin \DIFadd{simulation duration, the system is }\DIFaddend less dense than it should have been. In turn, our simulations have a strong initial mass loss\DIFaddbegin \DIFadd{, }\DIFaddend which adds to the amplitudes of the profile density peaks of Fig.~\ref{fig:stream_on_sky}. This inaccuracy is acceptable for the scope of this work. First\DIFdelbegin \DIFdel{is that }\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend Palomar~5's tails indeed have more mass than the cluster itself. \citet{2017ApJ...842..120I} reports that there could be three times as much mass in Palomar~5's tails as the cluster itself. Secondly, the exact form of the density distribution is less important than having a population of particles present that can probe a cluster flyby event.
    ...
    To \DIFdelbegin \DIFdel{find out }\DIFdelend \DIFaddbegin \DIFadd{determine }\DIFaddend which globular clusters were responsible \DIFdelbegin \DIFdel{of }\DIFdelend \DIFaddbegin \DIFadd{for }\DIFaddend creating these gaps \DIFdelbegin \DIFdel{, }\DIFdelend and when close passages \DIFdelbegin \DIFdel{took place}\DIFdelend \DIFaddbegin \DIFadd{occurred}\DIFaddend , we estimated the gravitational acceleration along the orbit of Palomar~\DIFdelbegin \DIFdel{5 and }\DIFdelend \DIFaddbegin \DIFadd{5. We }\DIFaddend represented it in the ($t$, $\tau$) space. $t$ is the simulation time\DIFaddbegin \DIFadd{, }\DIFaddend and $\tau$ \DIFdelbegin \DIFdel{is }\DIFdelend indicates how long it will take for Palomar~5 to reach \DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{a given point in its orbit }\DIFaddend or how long ago it passed\DIFdelbegin \DIFdel{, a given point on its orbit}\DIFdelend . The use of $\tau$ is advantageous because the growth of the stream is approximately linear in $\tau$. 
    \DIFaddbegin 

    \DIFaddend On the other hand, \DIFaddbegin \DIFadd{streams }\DIFaddend in physical space \DIFdelbegin \DIFdel{streams on eccentric orbits indeed grow in time yet the growth rate is modulated }\DIFdelend \DIFaddbegin \DIFadd{are modulated by their orbital eccentricity }\DIFaddend with periodic expansion and contraction depending on the orbital phase \citep[see the top panel of Fig.~5.][for an example]{2016MNRAS.457.3817S}. Adopting this \DIFdelbegin \DIFdel{time space, }\DIFdelend \DIFaddbegin \DIFadd{time-space }\DIFaddend and reporting the gravitational acceleration along \DIFaddbegin \DIFadd{the }\DIFaddend Palomar~5 orbit in this space, \DIFdelbegin \DIFdel{the identification of the globular clusters which }\DIFdelend \DIFaddbegin \DIFadd{identifying the globular clusters that }\DIFaddend produced the perturbation \DIFdelbegin \DIFdel{, }\DIFdelend and the time at which it occurred becomes \DIFdelbegin \DIFdel{straight forward}\DIFdelend \DIFaddbegin \DIFadd{straightforward}\DIFaddend . We refer the reader to Appendix~\ref{sec:Perturber_Identification} for \DIFdelbegin \DIFdel{an in-depth }\DIFdelend \DIFaddbegin \DIFadd{a detailed }\DIFaddend description of the procedure. In this way, we \DIFdelbegin \DIFdel{could }\DIFdelend \DIFaddbegin \DIFadd{can }\DIFaddend identify that the clusters responsible for creating gaps in the simulation of \DIFaddbegin \DIFadd{the }\DIFaddend Palomar~5 stream\DIFaddbegin \DIFadd{, as }\DIFaddend reported in Fig.~\ref{fig:decomposition}\DIFaddbegin \DIFadd{, }\DIFaddend are NGC~2808, NGC~7078\DIFaddbegin \DIFadd{, }\DIFaddend and NGC~\DIFdelbegin \DIFdel{104, and that their }\DIFdelend \DIFaddbegin \DIFadd{104. Their }\DIFaddend close passages occurred 200~Myr, -1.9\DIFaddbegin \DIFadd{, }\DIFaddend and -2.1~Gyr ago, respectively. 

    To further \DIFdelbegin \DIFdel{probe that }\DIFdelend \DIFaddbegin \DIFadd{investigate whether }\DIFaddend the three clusters above are responsible \DIFdelbegin \DIFdel{of }\DIFdelend \DIFaddbegin \DIFadd{for }\DIFaddend producing the gaps observed in this simulation, we conducted additional experiments by including only the perturbation of each \DIFdelbegin \DIFdel{of these three clusters }\DIFdelend \DIFaddbegin \DIFadd{cluster }\DIFaddend at a time, \DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{while }\DIFaddend neglecting the gravitational perturbations of all the other clusters in the Galaxy.
    ...
    The analysis presented in Fig.~\ref{fig:decomposition} has been repeated for the whole set of simulations, and \DIFdelbegin \DIFdel{is reported in Appendix~gallery\_of \_gaps }\DIFdelend \DIFaddbegin \DIFadd{Appendix~\ref{sec:gallery_of_gaps} reports all of them }\DIFaddend for completeness. For all \DIFdelbegin \DIFdel{of }\DIFdelend the streams, morphologically speaking, the only changes appear to be the existence of gaps or \DIFdelbegin \DIFdel{not}\DIFdelend \DIFaddbegin \DIFadd{their absence}\DIFaddend . We do not observe a thickening of the streams due to an increased velocity dispersion. 

    From this analysis\DIFaddbegin \DIFadd{, }\DIFaddend we can derive a statistical view of \DIFdelbegin \DIFdel{: }\DIFdelend 1) the number of gaps generated on the Palomar~5 stream by the \DIFdelbegin \DIFdel{whole }\DIFdelend system of Galactic globular clusters\DIFdelbegin \DIFdel{; }\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend 2) the clusters \DIFdelbegin \DIFdel{which }\DIFdelend \DIFaddbegin \DIFadd{that }\DIFaddend generated these gaps\DIFaddbegin \DIFadd{, }\DIFaddend and the time history of these perturbations. From this, we can then quantify \DIFdelbegin \DIFdel{: }\DIFdelend 3) the properties of the perturbers (their masses, sizes\DIFdelbegin \DIFdel{and their }\DIFdelend \DIFaddbegin \DIFadd{, and }\DIFaddend orbital parameters) as well as 4) the impact geometry of the encounters, which allows us to understand which encounters are more favorable \DIFdelbegin \DIFdel{to generate }\DIFdelend \DIFaddbegin \DIFadd{for generating }\DIFaddend gaps in the Palomar~5 stream. In the following, we will present \DIFaddbegin \DIFadd{the }\DIFaddend results of this analysis, \DIFdelbegin \DIFdel{by starting from }\DIFdelend addressing points 1) and 2).





  \subsection{The history and statistics of gap creations in \DIFaddbegin \DIFadd{the }\DIFaddend Palomar~5 stream}\label{sect:history}
    ...
    \begin{table}[h]
      \centering
      \DIFaddbeginFL \caption{\DIFaddFL{The number of gaps created by each perturber across all 50 simulations.}}
      \label{tab:gaps_per_perturber}
      \DIFaddendFL \begin{tabular}{|ll|ll|ll|}
      \hline
      ...
      \DIFdelbeginFL %DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{The number of gaps created by each perturber across all 50 simulations of Palomar~5's tidal tail. These data are color-coded and illustrated in Fig.~\ref{fig:histogram_impact_time}.}}
      %DIFAUXCMD
%DIFDELCMD < \label{tab:gaps_per_perturber}
%DIFDELCMD <     %%%
\DIFdelendFL \DIFaddbeginFL \tablefoot{These data are color-coded and illustrated in Fig.~\ref{fig:histogram_impact_time}.}
    \DIFaddendFL \end{table} 

    In total, we report the finding of 73 gaps \DIFdelbegin \DIFdel{in }\DIFdelend \DIFaddbegin \DIFadd{across }\DIFaddend our 50 simulations\DIFdelbegin \DIFdel{which is on average }\DIFdelend \DIFaddbegin \DIFadd{, which averages to }\DIFaddend 1.5 gaps \DIFdelbegin \DIFdel{/simulationand were created by 18 different perturbers . The distribution of }\DIFdelend \DIFaddbegin \DIFadd{per simulation. Eighteen different perturbers provoke the gaps. Table~\ref{table:gap_distribution} presents the distribution of the }\DIFaddend number of gaps appearing per simulation\DIFdelbegin \DIFdel{is presented in Table~\ref{table:gap_distribution}}\DIFdelend . If we consider NGC~2808 \DIFdelbegin \DIFdel{to be }\DIFdelend an outlier and exclude it from the experiment, we observe an average of 0.6 gaps per simulation.  

    \begin{table}[h]
      \centering
      \DIFaddbeginFL \caption{\DIFaddFL{Occurrence of gaps in Palomar~5 streams, in our simulations.}}\label{table:gap_distribution}
        \DIFaddendFL \begin{tabular}{|l|c|c|c|c|c|}
          \hline
        ...
        \DIFdelbeginFL %DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Occurrence of gaps in Palomar~5 streams,  in our simulations. More specifically, the table reports the number of simulations (second row) for which a given number of gaps (first row) is found. }}%DIFAUXCMD
%DIFDELCMD < \label{table:gap_distribution}
%DIFDELCMD <     %%%
\DIFdelendFL \DIFaddbeginFL \tablefoot{More specifically, the table reports the number of simulations (second row) for a given number of gaps (first row).}
    \DIFaddendFL \end{table}

    We need a more sophisticated statistic \DIFdelbegin \DIFdel{that can be compared to }\DIFdelend \DIFaddbegin \DIFadd{to compare our results to }\DIFaddend other simulations, so we turn to the \DIFdelbegin \textit{\DIFdel{gap creation rate}} %DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{gap creation rate }\DIFaddend developed by \citet{2012ApJ...748...20C}. The gap creation rate is the number of gaps that appear per \DIFdelbegin \DIFdel{time and }\DIFdelend \DIFaddbegin \DIFadd{unit time and is }\DIFaddend normalized by the \DIFdelbegin \DIFdel{length of the stream}\DIFdelend \DIFaddbegin \DIFadd{stream's length}\DIFaddend . For our simulations, this rate is given by: \begin{equation} \label{eq:gap_creation_rate} \mathcal{R}_{\textrm{Pal 5}} =  \frac{1}{T}\int_{0}^T l^{-1}(t) \sum_i \delta(t-t_i) dt,\end{equation}where $T$ is the total integration time, $l(t)$ is the length of the stream, and $\sum_i \delta(t-t_i)$ sums over the gap occurrences, with $i$ indexing over the number of gaps in a given simulation. Here, $\delta$ represents the Dirac delta function. This expression can be simplified to:\begin{equation}\mathcal{R}_{\textrm{Pal 5}} =  \frac{1}{T} \sum_i \frac{1}{l (t_i)}. \end{equation}

    This computation allows us to analyze the distribution of gap creation rates across all simulations. Notice that since the gap creation rate \DIFdelbegin \textit{\DIFdel{adds in parallel}}%DIFAUXCMD
\DIFdel{, naturally}\DIFdelend \DIFaddbegin \DIFadd{adds in parallel, naturally, }\DIFaddend gaps that occur at earlier times when the stream was shorter are weighted higher than those that occur when the stream is longer. \DIFdelbegin \DIFdel{The results are shown in }\DIFdelend Fig.~\ref{fig:gapcreationrate} \DIFdelbegin \DIFdel{. This }\DIFdelend \DIFaddbegin \DIFadd{presents these results which are }\DIFaddend is roughly consistent with a simple estimate of the average gap creation rate: with 73 gaps over 5~Gyr of integration time for a stream about 20 kpc in length, the naive estimate is approximately 0.015~km~s$^{-1}$~kpc$^{-2} $ (which is roughly equivalent to $0.015~\rm{Gyr^{-1}kpc^{-1}}$). This \DIFaddbegin \DIFadd{naive estimate }\DIFaddend is about double the weighted mean gap creation rate of 0.009~km~s$^{-1}$~kpc$^{-2}$ \DIFdelbegin \DIFdel{. The naive estimate }\DIFdelend \DIFaddbegin \DIFadd{and }\DIFaddend is higher because it does not account for the growth of the stream over time, unlike Eq.~\ref{eq:gap_creation_rate}.
  ...
  \subsection{Impact geometry and parameters of the perturbers}\label{sect:geometry}
    ...
    \begin{figure*}
      ...
      \caption{The distribution and relationship between the impact variables from Eq.~\ref{eq:change_in_momentum} for all close fly-bys considered. The \DIFaddbeginFL \DIFaddFL{colors indicate the }\DIFaddendFL encounters that cause gaps \DIFaddbeginFL \DIFaddFL{and }\DIFaddendFL are \DIFdelbeginFL \DIFdelFL{labeled with }\DIFdelendFL the same \DIFdelbeginFL \DIFdelFL{colors }\DIFdelendFL as \DIFdelbeginFL \DIFdelFL{from }\DIFdelendFL Fig.~\ref{fig:histogram_impact_time}\DIFdelbeginFL \DIFdelFL{in the scatter plot}\DIFdelendFL , with white \DIFdelbeginFL \DIFdelFL{rings }\DIFdelendFL edges for visibility \DIFdelbeginFL \DIFdelFL{, }\DIFdelendFL and binned in red for the histograms. The \DIFaddbeginFL \DIFaddFL{black histogram shows the }\DIFaddendFL close encounters that did not lead to gaps\DIFdelbeginFL \DIFdelFL{are shown in black}\DIFdelendFL . Indeed, no obvious trend \DIFdelbeginFL \DIFdelFL{delimiting }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{has emerged that delimits }\DIFaddendFL these planes into \DIFdelbeginFL \DIFdelFL{gap producing }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{gap-producing }\DIFaddendFL or \DIFdelbeginFL \DIFdelFL{not emerges}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{non-gap-producing ones}\DIFaddendFL .}
      \label{fig:impact_geometry_statistics}    
    \end{figure*}



    With the perturbers identified, we perform \DIFdelbegin \DIFdel{statistics in the pursuit of understanding what conditions are }\DIFdelend \DIFaddbegin \DIFadd{statistical analysis to understand the conditions }\DIFaddend necessary for a globular cluster to induce a gap \DIFdelbegin \DIFdel{on }\DIFdelend \DIFaddbegin \DIFadd{in }\DIFaddend the Palomar~5 stream. We turn to impact theory, which in its simplest form is presented in works such as \citet{2008gady.book.....B}. Consider two particles: one stationary and the other moving past it. The \DIFaddbegin \DIFadd{impact parameter is the }\DIFaddend distance between the two particles at \DIFaddbegin \DIFadd{the point of }\DIFaddend their closest approach\DIFdelbegin \DIFdel{is known as the impact parameter. To simplify the analysis, the }\textit{\DIFdel{impulse approximation}} %DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{. The impulse approximation }\DIFaddend is employed, which assumes that the velocity of the perturber remains unchanged during the interaction. This assumption simplifies the computation.
...
models their perturber }\DIFaddend as a Plummer sphere, like in our simulations. Since \DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{a }\DIFaddend stream is not \DIFdelbegin \DIFdel{zero-dimensional and has length , both }\DIFdelend \DIFaddbegin \DIFadd{a point but has length and an orientation in space, one needs to consider }\DIFaddend the parallel and perpendicular components of the \DIFdelbegin \DIFdel{perturber velocity must be considered}\DIFdelend \DIFaddbegin \DIFadd{velocity to describe the impact fully}\DIFaddend . Consequently, five parameters determine the change in velocity of a given stream particle: $M$, $r_p$, $b$, $W_\parallel$, and $W_\perp$, which are the \DIFdelbegin \DIFdel{: }\DIFdelend mass of the perturber, size of the perturber, impact parameter, parallel and perpendicular components of the relative velocity. As detailed in Appendix~\ref{sec:reconstruction}, we calculated these parameters for all our \texttt{full} simulations \DIFdelbegin \DIFdel{, }\DIFdelend by selecting -- for each of them --  the strongest \DIFdelbegin \DIFdel{5 }\DIFdelend \DIFaddbegin \DIFadd{five }\DIFaddend flybys of a perturber with the Palomar~5 stream. Thus, we compute \DIFdelbegin \DIFdel{a total of  }\DIFdelend 250 impacts \DIFdelbegin \DIFdel{, }\DIFdelend and flag those that give way to gaps. 
    ...
    Note that \DIFdelbegin \DIFdel{in this figure , we show }\DIFdelend \DIFaddbegin \DIFadd{this figure displays }\DIFaddend the total relative velocity rather than separating \DIFaddbegin \DIFadd{the }\DIFaddend parallel and perpendicular components, \DIFdelbegin \DIFdel{because from a visual inspection, we could not find any specific trends when }\DIFdelend \DIFaddbegin \DIFadd{as no specific trends were observed when plotting }\DIFaddend the two velocity components \DIFdelbegin \DIFdel{were plotted }\DIFdelend separately. We also \DIFdelbegin \DIFdel{exclude }\DIFdelend \DIFaddbegin \DIFadd{excluded }\DIFaddend the characteristic cluster radius, \DIFdelbegin \DIFdel{as it }\DIFdelend \DIFaddbegin \DIFadd{which }\DIFaddend showed little correlation with the results, likely due to the narrow range of globular cluster radii (see Fig.~\ref{fig:mass_size_plane}). This factor might be more significant for dark matter sub-halos, where size variation is greater.

      
  
    While Fig.~\ref{fig:impact_geometry_statistics} demonstrates that mass, relative velocity, or impact parameter alone cannot predict gap formation, one interesting result emerges: \DIFdelbegin \DIFdel{no gaps are created from interactions with }\DIFdelend impact parameters greater than 300~pc \DIFaddbegin \DIFadd{do not create gaps}\DIFaddend . The stream widths are roughly 200~pc, as seen in Appendix~\DIFdelbegin \DIFdel{gallery\_of\_gaps}\DIFdelend \DIFaddbegin \DIFadd{\ref{sec:gallery_of_gaps}}\DIFaddend . This finding is even more evident when examining the $b$-$M$ plane. A series of perturbers at roughly $\sim8 \times 10^5 M_\odot$ highlights NGC~2808's flybys, where all encounters with impact parameters under 200~pc result in gaps, while those beyond this distance do not. In other words, even the most massive \DIFdelbegin \DIFdel{globulars }\DIFdelend \DIFaddbegin \DIFadd{globular clusters }\DIFaddend with masses greater than about $10^6 M_\odot$ cannot cause gaps if their impact parameters are greater than \DIFdelbegin \DIFdel{about }\DIFdelend \DIFaddbegin \DIFadd{roughly }\DIFaddend $300$~pc. \DIFdelbegin \DIFdel{It is interesting to note that for values of the impact parameter below this threshold}\DIFdelend \DIFaddbegin \DIFadd{Interestingly}\DIFaddend , even fast encounters ($\delta v > 300$~km/s) can produce gaps \DIFdelbegin \DIFdel{. Perhaps, }\DIFdelend \DIFaddbegin \DIFadd{for impact parameter values below this threshold. Perhaps }\DIFaddend this is not surprising\DIFaddbegin \DIFadd{, }\DIFaddend since the range of possible relative velocities is much less than \DIFaddbegin \DIFadd{that of }\DIFaddend mass and impact \DIFdelbegin \DIFdel{parameter which vary }\DIFdelend \DIFaddbegin \DIFadd{parameters, which vary by }\DIFaddend two and three orders \DIFdelbegin \DIFdel{, respectively while }\DIFdelend \DIFaddbegin \DIFadd{of magnitude, respectively. In contrast, }\DIFaddend the relative velocities only vary by about a factor of three.

    \begin{figure}
      \centering
      \includegraphics[width=1\linewidth]{impact_geometry_statistics_deltaP.png}
      \caption{The distribution of imparted change in momentum (per unit mass) from a cluster flyby given by Eq.~\ref{eq:change_in_momentum}. The data set includes the top 5 strongest flybys from each simulation. Those that cause gaps are colored, stacked, and \DIFdelbeginFL \DIFdelFL{overlain }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{overlaid }\DIFaddendFL atop \DIFdelbeginFL \DIFdelFL{of }\DIFdelendFL those that \DIFdelbeginFL \DIFdelFL{do--the distribution of which }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{do not — this }\DIFaddendFL is \DIFdelbeginFL \DIFdelFL{outlined in grey}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{the gray distribution}\DIFaddendFL . Note that the meaning of the colors is the same as in Fig.~\ref{fig:histogram_impact_time}.}
      \label{fig:deltap}
    \end{figure}
...
\citet{2017MNRAS.470...60E} }\hskip0pt%DIFAUXCMD
also present a study about the impact of globular clusters in producing gaps on the Pal~5 stream. In section~6.3 of their article, \mbox{%DIFAUXCMD
\citet{2017MNRAS.470...60E} }\hskip0pt%DIFAUXCMD
...
\section*{Data Availability}
  ...
  The code used to produce the data in these simulations is publicly available at https://github.com/salvatore-ferrone/tstrippy. Examples \DIFdelbegin \DIFdel{on the codes }\DIFdelend \DIFaddbegin \DIFadd{of the }\DIFaddend installation and use \DIFaddbegin \DIFadd{of the code }\DIFaddend are provided at https://tstrippy.readthedocs.io. 
\DIFdelbegin \DIFdel{The simulations will also be made publicly available at etidal-project.obspm.fr.
}\DIFdelend 
...
%DIF <  or another style like plain, unsrt, etc.
\bibliography{bibliography} 
%DIF <  replace 'references' with the name of your .bib file
...
\begin{appendix}
  ...
  \section{Gap \DIFdelbegin \DIFdel{Detection}\DIFdelend \DIFaddbegin \DIFadd{detection}\DIFaddend } \label{sec:gap_detection}
...
\section{\DIFadd{Survival of gaps in streams emerging from more massive globular clusters}}\label{sec:gaps_vs_gcmass}
    ...
    \DIFadd{Fig.~\ref{fig:decoherence_shocked_outflow} dissects the more massive stream of Fig.~\ref{fig:gap_no_more} and instructs us why the gap does not persist. The bottom panel shows the total density distribution and dissects it into two profiles: stars that are either leaked or shocked out of the cluster. By shocked, we mean the stars that are lost during pericenter passages. Tidal forces generally scale with $r^{-3}$, thus significantly strengthening through their pericenter passages (See Fig.~3 of \mbox{%DIFAUXCMD
\citet{2004AJ....127.2753D} }\hskip0pt%DIFAUXCMD
for an illustration of the magnitude of the tidal forces along an orbit). A consequence of the pericenter passages is that many more stars escape, fleeing with a higher mean velocity and velocity dispersion, which can be visualized in the Christmas tree illustration in Fig.~A.3 of \mbox{%DIFAUXCMD
\citet{2012A&A...546L...7M}}\hskip0pt%DIFAUXCMD
. We consider the continuous outflow of stars between pericenter passages to be leaked stars. We identify pericenter passages by computing each local minimum of the cluster's galactic-centric 3D radius with }\texttt{\DIFadd{scipy.signal.find\_peaks}}\DIFadd{. The duration of each shock is: $t_{\textrm{shock}}$ = 2$\sqrt{r/|\nabla\Phi|}$, where $r$ is the galactocentric radius and $|\nabla\Phi|$ is the magnitude of the acceleration from the galactic potential. Stars within this range about a given pericenter passage are all given the same color and shown as individual groups in the first, second, third, and fourth panels of Fig.~\ref{fig:decoherence_shocked_outflow}.
}
