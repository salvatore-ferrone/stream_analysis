...
\section{Results}
  ...
  \subsection*{Overview}
    ...
    One striking result is that more than half of the gaps observed are caused by NGC2808. Upon examining the figures in the appendix, we note that these gaps are of similar size and occur in almost the same position in the leading tail across most simulations. This is no coincidence, as the size of a gap is a function of time, and recent encounters allow only limited time for the gap to develop. As shown in Fig.\ref{fig:histogram_impact_time}, the gaps caused by NGC2808 result from a flyby that occurred approximately 200 Myr ago. It is expected that a recent flyby appears in most simulations, as uncertainties grow when we look further back in time, allowing broader variation in the phase space positions of the clusters. However, due to relatively low uncertainties associated with NGC2808 and Palomar 5 compared to the integration time, the recent flyby of NGC~2808 occurs in 44 out of 50 simulations. Although the flybys do not occur identically in each simulation, the range in gap contrast, shown in the appendix, indicates that closer encounters lead to more significant impacts.
      

    Additionally, we observe that most gaps occur in the leading tail. We believe this to be a coincidence, as stars escape from the globular cluster's Lagrange points at roughly equal rates. The two main differences are the orbital phase and that the leading tail is about 400~pc closer to the Galactic center than the trailing tail. However, this difference is negligible compared to the orbital radius of 8-12kpc. As such, we expect the likelihood of a gap appearing in either tail to be equal, with the presence of one gap not influencing the probability of another. 
