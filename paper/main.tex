...
\section{Results}
  ...
  \subsection*{Overview}
    This experiment follows a Monte Carlo scheme, where we sample the initial conditions of the globular clusters' masses and kinematics based on their reported observational uncertainties. We repeat the experiment multiple times, running 50 simulations to balance computational cost with the exploration of possible outcomes. The uncertainties in the globular clusters' parameters are significant enough that, over the 5 Gyr integration period, they could occupy various regions of their phase space given their initial energy and angular momentum. After inspecting all simulations, we present one (Fig.~\ref{fig:stream_on_sky}) chosen for its clear and prominent gaps. Two of these gaps are visible in galactic coordinates, and they become even more apparent when we marginalize over latitude to view the 1D profile density along the longitude.
  
    For each simulation, we follow the methods outlined earlier. We simulate the creation of the Palomar 5 stream for 50 different samplings of the initial conditions in two environments. In the \texttt{vanilla} simulations, the star particles only feel the gravitational influence of the galaxy and Palomar 5. In contrast, the \texttt{full} simulations include the effects of all globular clusters on the star particles.

  
    Through a comparative analysis, we explore the influence of the globular clusters on stellar streams under conditions of greater clarity than reality offers. In this controlled setting, we avoid challenges like distinguishing stream stars from field stars and do not need to define gaps independently. As numerical modelers, we can "play god" and run the same experiment with or without globular clusters, making it easier to isolate their effects.



    This analysis is illustrated in Fig.~\ref{fig:profiles} we identify regions where the full simulations show significantly lower densities than the vanilla simulations. Many gaps are clearly visible, while some subtler under-densities only appear when examining the 1D profiles. All simulations and labeled gaps are presented in Appendix~\ref{appendix:gallery_of_gaps}

    Next, we followed the method in section~\ref{sec:SuspectToCulprit} to identify which globular clusters caused the gaps. This approach uses the orbit of Palomar 5 as a proxy for the stream. We computed the gravitational force from all globular clusters at various segments along Palomar 5's orbit for the entire simulation. The five strongest flybys were identified, and the corresponding clusters labeled as "suspects." Fig.~\ref{fig:force-on-orbit} shows this: the top panel displays the force exerted by the globular clusters on Palomar 5's orbit, while the bottom panel shows the stream density. A density threshold was imposed to quantify the stream's length. We cross-referenced the impact locations with the gaps to determine which cluster was responsible. For example, gaps in the leading tail correspond to suspects with $\tau>0$.
    
    The top panel of Fig.\ref{fig:force-on-orbit} reveals interesting periodic patterns in the $(t,\tau)$ plane. This is due to pericenter passages when Palomar 5 approaches the center of mass of the globular cluster system. Notably, for the collisions from NGC104 and NGC7078, we observe wakes in the density map, which confirm their role in creating gaps. However, the strongest peak in gravitational force does not always result in a gap. For example, NGC~5272 exerts the greatest force, but it does not cause a gap. This is because the change in momentum—not just the force—matters. A brief, strong force may have little effect on the stream, especially with an offset of about 200 pc between the stream and the orbit in tail coordinates. Despite this, the five strongest impacts account for all gaps except in Sampling-014 (Fig.\ref{fig:gallery3}), where a gap caused by NGC~6584 was linked to the 7th strongest peak. In some cases, we ran additional simulations with only a single suspect to confirm its role in creating a gap. An example of this is shown in Fig.\ref{fig:decomposition}.
  ...
  \subsection*{Gap Morphology}
    We analyze the structure of streams impacted by globular clusters. The gallery of gaps, found in the appendix, includes results from all the simulations considered in this experiment.

    How do globular cluster passages affect the stream? Much of this has been well studied, as discussed in Sanders et al. (2016). They even considered different types of impactors, such as those with varying density profiles (e.g., Hernquist, Plummer, NFW, and truncated NFW). While there are some minor differences between these impactors, the overall conclusion remains the same: impacts from small, dense perturbers, such as globular clusters, only affect the stream locally.
    
    Our results align with previous findings—perturber crossings cause local perturbations in the stream without altering its global characteristics. For instance, the stream does not become warmer or thicker, changes that have been linked to other types of interactions. For example, [AUTHOR] demonstrated that XXX can do YYY. As described by Erkal et al. (2015), perturbers pull particles from the sides toward the impact point, creating an under-dense region at the center of the gap, with overdensities piling up at the shoulders.


    Interestingly, most of the gaps we observe occur in the leading tail. However, we believe this to be coincidental. Stars escape from the globular cluster through the two Lagrange points at equal rates (cite?). The lengths of the tails are also equal, with only a small difference of approximately 400 pc in radial distance from the galactic center. This difference is negligible compared to the orbital radius of about 10 kpc. Therefore, we expect the probability of a gap appearing in the leading or trailing tail to be equal, and the presence of one gap should not influence the likelihood of another.

    Yet, the observation of only 8 gaps in the trailing tail out of 73 total gaps is inconsistent with this expectation. However, 44 of these gaps are associated with NGC2808. As shown in Fig.\ref{fig:histogram_impact_time}, all of these gaps result from the same recent encounter. We thus discard NGC~2808 for the current analysis, as its gaps violate the assumption of independent events. After excluding these, we are left with 21 gaps in the leading tail and 8 in the trailing tail across the remaining simulations. The probability of observing 8 or fewer gaps in 29 trials, assuming each has a 50\% chance, is approximately 1.2\%. While this outcome is unlikely, it is not impossible.


    All of the gaps are oblique in tail coordinates. This is an artifact of using tail coordinates and arrises because we project the star-particles onto the nearest point of the orbit. (this is really difficult to explain with words and super simple with a picture. The picture being that we have three evenly spaced parallel lines that are close together. Then, a third point that is far away.  )
  
    Do our gaps grow linearly in time? what about this square root stuff? 

    Lastly, since the streams have width, there are some instances where the stream is not significantly impact across this width. For instance, the gaps from NGC~5284 in Sampling 019 and NGC~6333 in Sampling 033 are more akin to bites than gaps.
  ...
  \subsection*{Perturber Statistics}
    ...
    The clusters that impacted Palomar 5's stream in our simulations are presented in Fig.~\ref{fig:histogram_impact_time}, along with when they impacted the stream. First, there are few impacts that happen early on in the simulation. This is not surprising, as it the tidal tails are smaller and thus there is less volume to be perturbed. Next, is that 44/50 simulations contain a visible gap from NGC2808, all of the encounters occuring about 200~Myr ago. This is so much more than any other peak that we broke the scale of the histogram so that this peak would not dwarf the significance of all others. The reason is that the uncertainty in the precise position of the clusters along their orbits grows in look back time. Since this encounter is recent, most of the sampled initial conditions give rise to a significant enough impact in the stream that causes a gap. 

    \begin{figure*}
      \centering
      \includegraphics[width=\linewidth]{histogram_impact_time.png}
      \caption{The time of impact for each flyby that caused a gap. Each cluster is given a consistent and unique color. The scale of the histogram is broken between 8 and 42 since the gaps from NGC~2808 are in a league of their own with respect to other clusters.}
      \label{fig:histogram_impact_time}
      \end{figure*}    
  
    We desire to search for some generalizable characterisitcs of the perturbers to see what contrasts them to globular clusters that do not impact the Pal5 Stream. We present three different graphs in Fig.~\ref{fig:mass_size_plane}. The top plot shows the angular momentm and energy plane. To no surprise, the globular clusters that perturb Palomar 5 are within a band of energy enclosed the energy of Palomar 5. This indicates that a cluster's orbital space needs to overlap with Palomar 5's. There appears not to be a preference in angular momentum. Note that in this panel all initial conditions for the clusters are plotted. Clusters of gray dots indicate the same cluster. However, for the least bound milky way clusters, the uncertainties in distances and motions allow for a wide range of energies, prograde and retrograde orbits. These are Crater, Palomar X and Eridanus. This is consistent with Vasiliev et al 2021, yet not very significant for this study since they do not influence Palomar 5 and could have been ignored. 
    
    Next, in the middle panel of Fig.~\ref{fig:mass_size_plane} we view the mass size plane. The
