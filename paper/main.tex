...
\section{Methods}
  ...
  \subsection{Numerical Methodology}
    ...
    During the integration, intermediate snapshots were saved to facilitate the analysis of stellar streams and the effects of cluster impacts. Specifically, for each of the 50 realizations of the Palomar 5 stream, we saved $5000$ intermediate timesteps which is a temporal resolution of 1 million years. To provide an estimate of the size of our simulations, we have $N_p=100 000$ number of particles, $N_\textrm{time stamps} = 5 000$ time steps, $N_\textrm{phase}=6$ phase space coordinates, $N_\textrm{sampling}=50$ monte-carlo samplings, and using single precision floating point numbers, $\textrm{Single}=4$~bytes. The product of the aforementioned terms results in approximately: $N_p=100 000 \times N_\textrm{time stamps}\times N_\textrm{phase}\times N_\textrm{sampling} 4 \textrm{bytes}\approx$ 600 GB of data.




  \subsection{Gap detection and Characterization} \label{sec:gap_detection}
    ...
    To facilitate this process, we ran a series of \texttt{reference} simulations. In these control simulations, we used the same 50 sets of initial conditions but excluded interactions between globular clusters and omitted the gravitational influence of the clusters on the star particles within the stream--save for the host cluster Palomar 5. By comparing the morphological differences between these control simulations and the \texttt{full} simulations (which included cluster interactions), we could identify and assess the impact of globular clusters on the stream structure more effectively. 
    ...
    More details on how the gaps and corresponding perturbing globular clusters were identified is provided in Appendix.~XXX. A first order approach of understanding the significance of an impact one needs to compute its change in momentum, which is determined by integrating the force acting on the particle over the duration of the interaction. A useful approximation for this change in momentum is the force at the closest approach multiplied by an estimate of the interaction time:
    \begin{equation} 
      \Delta v \approx \text{Force} \times \text{interaction time} = \frac{GM}{b^2} \times 2\frac{b}{v} = 2\frac{GM}{bv}, 
      \end{equation} \label{eq:change_in_momentum}
    where $M$ is the mass of the perturber, $m$ is the mass of the particle, $b$ is the impact parameter, $v$ is the relative velocity of the perturber with respect to the particle, and $G$ is the gravitational constant. We compute this for the top five cloestest approaches of all globular clusters on an orbital segment about Palomar 5 that approximates the stream through out each of the 50 simulations. In the end, we have a sample of 250 impacts and compare the impact geometry of those that cause gaps to those that do not. 
...
\section{Results}
  ...
  \subsection*{Overview}
    ...
    This experiment follows a monte-carlo scheme, where we sample the initial conditions of the globular clusters' masses and kinematics based on their corresponding uncertainties and then repeat the experiment multiple times. We choose to perform the simulation 50 times to balance computational cost with a high enough number to allow us to explore a range of possible outcomes. The uncertainties on the globular clusters are large enough that over the course of the 5 Gyr integration period, they could be anywhere allowed in their phase space given their initial energy and angular momenta. We inspected all simulations upon their completion and present one of them in in Fig.~\ref{fig:stream_on_sky}--which we selected for its clear and prominent gaps. Two of these gaps are visible in galactic coordinates and become even more apparent when marginalizing over latitutde to view it's 1D profile density over longitude. 

    For each completed simulation, we follow the scheme laid out in the methods. I.e. we simulate the creation of the Palomar 5 stream for 50 different samplings of the initial conditions, with two different cases. The \textit{Full} simulation that includes the influence of all globualr clusters on the star particles representing Palomar 5 and its stream, as well as the \textit{vanilla} simulations in which the star-particles feel only the galaxy and Palomar 5. We do this for we attempt to quantify the number of gaps from globular clusters in the limit of maximum information. I.e. we are not limited by contamination of field stars, and since we are computing our own simulations, we know what a gapless stream should look like. 
...
\begin{appendix}
  ...
  \section{Gap detection and characterization methods}
