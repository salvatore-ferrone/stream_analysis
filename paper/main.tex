...
\newcommand{\salvatore}[1]{\textcolor{orange}{{#1}}}
...
\section{Introduction}
  ...
  % \paola{Postulated about a century ago, to explain the large velocity dispersions of galaxies inside clusters \citep[see][]{zwicky33}\footnote{The English translation of the original paper is available at \citet{zwicky_EN}} and the rotation curves of disk galaxies as well \citep{freeman70, rubin70}, different  astronomical measurements conducted over the last century point to the existence of dark matter, from temperature fluctuations in the Cosmic Microwave Background  to weak and strong gravitational lensing \citep[see, among many, ][]{planck20, mandelbaum06, hsueh20}. Dark matter constitutes the dominant ($\sim$~85\%) contributor to matter in the Universe \citep{planck20}, and shapes the building-up of its structures over time \citep{padmanabhan93, bertschinger98, springel05}.}

  Over a century ago \citet{zwicky33}\footnote{The English translation of the original paper is available at \citet{zwicky_EN}} postulated the existence of dark matter to explain the excessively large velocity dispersions of galaxies within their galaxy clusters. Since this time, mounting evidence has been assembled to support this claim. \citet{freeman70} and \citet{rubin70} provided some of the first measurements of the rotation curves of disk galaxies that are too fast moving to be driven by ordinary matter alone. The temperature fluctuations in the Cosmic Microwave Background radation are well explained by the presence of dark matter, which is well studied and documented by the Planck Mission, a review of which is presented by \citet{planck20}. The location and mass of dark matter on the large scale can be inferred and spatially mapped through strong and weak gravitational lensing, as demonstrated in \citet{planck20, mandelbaum06, hsueh20}. Dark matter contributes $\sim$~85~\% of the matter in the universe \citep{planck20} and is crucial for driving the hierarchical growth of ordinary in the universe. See \citet{padmanabhan93} for XXX, \citet{bertschinger98} for XXX and \citet{springel05} for XXX. 

  \salvatore{Why cite freeman70? This paper reports that spiral galaxys had disks with exponential decaying surface density profiles. }

  Despite significant progress, the true nature of dark matter remains elusive, and scientists continue to search for evidence of its existence beyond its gravitational effects on cosmic and galactic scales \citep[see][ for a review]{arbey21}. A key focus today is detecting dark matter on sub-galactic scales, because at these scales different cosmological scenarios predict different lumpiness of the halos \citep{madau08, zavala19}. \paola{One of the most promising tools to set limit to the halo lumpiness, or granularity, and hence to the number and mass density of the so-called sub-halos, is provided by the study of} stellar streams.
  
  Stellar streams, several-kpc long structures formed by the tidal disruption of globular clusters or dwarf galaxies orbiting a host galaxy, present \paola{indeed} a promising method for probing dark matter distribution. These tidal forces arise due to differential gravitational pulls across extended objects, causing stars farther from the galactic center to lag behind while those closer are pulled away. This stretching creates two tidal tails that trace the cluster's orbit, \paola{unless in the closest vicinity to the object \citep{montuori07}}. Numerical predictions of this phenomenon exist \paola{since already the '70s of the last century \citep[see, for example][]{keenan75}, well}  before the first detections of \paola{stellar streams around some Galactic globular clusters made by}  \citet{carl_j_grillmair_globular_1995}, \paola{and the nearly contemporary discovery of the Sagittarius stellar stream, the closest (to the Sun) example of a stream emerging from a dwarf satellite currently interacting with the Milky Way \citep{mateo98, newberg02, majewski03, martinez04}.} \paola{Subsequent works \citep[see][]{lehmann97, testa00, leon00, siegel01, lee03} extended Grillmair's findings to other globular cluster streams, until the discovery made by \citet{odenkirchen01,odenkirchen02, odenkirchen03} of long and thin tails outside the Palomar~5 globular cluster, which were shown to be extended for more than $10^\circ$ degrees in the sky.  These early works stimulated a vigorous search for stellar streams in the Galaxy in the following years \citep{grillmair06a, grillmair06b, belokurov06, grillmair09}, which has now entered a new dimension thanks to the Gaia astrometric mission \citep{prousti16}, which allows to look for these structures coupling photometry, astrometry and (when possible) spectroscopic  information. Currently about 20 GCs have been found to possess streams around them \citep[see][ for a summary of these discoveries]{mateu23}, while some streams do not have any clear association with existing GCs, pointing to the early presence of a population of clusters  now completely dissolved in the Galactic field \citep{ibata24, martin22, wan20}.}

  \paola{One of the interests in studying stellar streams, is that they } can constrain the gravitational field of their host galaxies, particularly the Milky Way. For example, \citet{varghese2011stellar} demonstrated how stellar streams can infer the mass and scale parameters of \paola{dark matter} haloes using various amounts of observational data, from basic right ascension and declination to full six-dimensional phase space information. \paola{The use of single streams for constraining the Galactic potential has led, however, to some ambiguous and not converging results over time. For example, \citet{law10} made use of the Sagittarius stream to infer that the dark matter halo of our Galaxy has a triaxial shape \citep[but see also][]{helmi04, johnston05, law05}; \citet{bovy16} concluded that the dark matter halo of our Galaxy is nearly spherical at the radii of the Palomar~5 and GD~1 streams. } Meanwhile, \citet{bonaca14} employed an information-theoretic approach, identifying the orbits and configurations of stellar streams that yield the most information about the galactic potential.
  
  \paola{As previously stated, }  the Gaia mission, and its Data Releases 2 and 3 \citep{gaia18, gaia23}, revolutionized the observational study of stellar streams, \paola{unraveling a population of stellar streams much more numerous than that previously known \citep[see, for example][]{malhan18}}, enabling significant advances in the field. Malhan et al. developed the StreamFinder algorithm, which identified 90 stellar streams within the Milky Way, some of which have been followed up by other instruments and surveys. The best-observed streams, such as GD-1 and Palomar 5, contain between 1,000 and 3,000 stars.  Leveraging this dataset, \citet{ibata24} constructed the most accurate model of the Milky Way to date based on a set of about 90 streams.

  In this study, we build on this body of work by investigating another important use case for stellar streams: probing dark matter on sub-galactic scales. According to simulations by Springel et al. (2008), $\Lambda$-Cold Dark Matter ($\Lambda$CDM) predicts that galaxies grow hierarchically, with dark matter clumps forming at a wide range of masses and sizes. These clumps, or \textit{subhalos}, are predicted to follow a mass distribution with a power-law slope slightly shallower than -2.0. To date, the smallest observed dark matter halo was detected through gravitational lensing in an Einstein ring, as described by \citet{vegetti12}. However, some models predict that dark matter clumps could exist down to the mass of Earth-like planets at least \citep[see][ and discussion in \citet{amorisco21}]{green05, wang20}. 
...
\section{Methods}
  ...
  \subsection{Numerical Methodology}
    Our numerical methodology follows the approach outlined in Ferrone et al. (2023), which we present here for completeness. We begin by extracting the phase space properties (positions, velocities) as well as the masses and half-mass radii of 165 globular clusters from the galactic globular cluster catalog by Baumgardt and Vasiliev et al. We handle the uncertainties through a monte-carlo approach. The catalog provides uncertainties for mass, distances, line of sight velocities, proper motions and includes a covariance term between the proper motions. We repeate the whole experiment 50 times, for each of which we vary the initial conditions of each cluster by sampling its five-dimensional gaussian uncertaintiy distribution. However, for the first simulation, which we name \texttt{Sampling 000}, we use the most probable values for the initial conditions. 
    ...
    The primary methodological departure from Ferrone et al. (2023) is that now we include globular cluster interactions. Instead of treating the clusters as point masses that only experience the gravitational potential of the galaxy, we now compute N-body interactions between the clusters during the backward integration over 5 billion years. The equations of motion for the globular cluster system are thus: 
    \begin{equation}
    ...
    During the integration, intermediate snapshots were saved to facilitate the analysis of stellar streams and the effects of cluster impacts. Specifically, for each of the 50 realizations of the Palomar 5 stream, we saved $5000$ intermediate timesteps which is a temporal resolution of 1 million years. To provide an estimate of the size of our simulations, we have $N_p=100 000$ particles, $N_\textrm{time stamps} = 5 000$ time steps, $N_\textrm{phase}=6$ phase space coordinates, $N_\textrm{sampling}=50$ monte-carlo samplings, and using single precision floating point numbers, $\textrm{Single}=4$~bytes. The product of the aforementioned terms results in approximately: $N_p \times N_{\textrm{time stamps}}\times N_{\textrm{phase}}\times N_{\textrm{sampling}} \times 4 \textrm{bytes}\approx$ 600 GB of data.
  ...
  \subsection{Gap Detection and Characterization} \label{sec:gap_methods}
    ...
    To facilitate this process, we conducted a series of \texttt{reference} simulations. These control simulations used the same 50 sets of initial conditions but excluded interactions with globular clusters, except for the host cluster, Palomar 5. In other words, the gravitational influence of all other clusters on the star particles within the stream was omitted. By comparing the morphological differences between these control simulations and the \texttt{full} simulations (which included cluster interactions), we could more effectively identify and assess the impact of globular clusters on the stream's structure. Details on gap identification are provided in Appendix~\ref{sec:gap_detection}, and the identification of corresponding perturbing globular clusters is detailed in Appendix~\ref{sec:Perturber_Identification}.


    Comparisons between the \texttt{reference} and \texttt{full} simulations were performed in the tail coordinate system. This system allows for consistent morphological comparisons by aligning the stream with the cluster's orbit. The coordinate system is based on the work of Dehnen et al. (2004) and is detailed in Appendix~\ref{appendix:TailCoordinates}. Briefly, the x' coordinate represents the position of a particle along the orbit relative to the globular cluster—whether it is ahead or behind the cluster. The y' coordinate measures the particle's distance within the orbital plane, where positive values indicate that the particle is farther from the Galactic center and negative values indicate that it is closer.

    
    Once the gaps and perturbers were identified, we sought to characterize the properties of the globular clusters that caused gaps and differentiate them from those that did not. Carlberg (2012) initiated the theory of gap creation in stellar streams due to gravitational flybys. This analysis was extended both analytically and numerically by Erkal et al. (2015), who studied uniform-density streams on circular orbits. Sanders et al. (2016) presented the most comprehensive theory, generalizing Erkal et al.'s work to include impacts on streams of varying densities on inclined and eccentric orbits using an action-angle approach.

    The significance of an impact is measured by the change in momentum imparted to the stream stars by the perturber. To first order, we follow the developments from collision theory as laid out in Galactic Dynamics (Binney \& Tremaine) and similar works by John Taylor. In the simplest case, the perturber is considered unperturbed during the interaction, and thus has constant velocity. We can consider the most basic scenario of one point mass impacting another. A useful approximation for the change in momentum is the force at the closest approach multiplied by an estimate of the interaction time: \begin{equation} \Delta v \approx \text{Force} \times \text{interaction time} = \frac{GM}{b^2} \times 2\frac{b}{v} = 2\frac{GM}{bv}, \end{equation} where $M$ is the mass of the perturber, $b$ is the impact parameter, $v$ is the relative velocity of the perturber with respect to the particle, and $G$ is the gravitational constant. We compute this for the five closest approaches of all globular clusters throughout each of the 50 simulations. Details on how this computation was performed are given in Appendix~\ref{sec:reconstruction}. Ultimately, we obtained a sample of 250 impacts and compared the impact geometry of those that caused gaps with those that did not.
...
\section{Results}
  ...
  \subsection*{Overview}
    ...
    One striking result is that more than half of the observed gaps are caused by NGC 2808, which had a flyby about 200 Myr ago. In fact, 44 of the 50 simulations show a gap from this interaction. Upon examining their morphology (Appendix~\ref{sec:gallery_of_gaps}), we observe that these gaps are consistently similar in size and location within the leading tail across most simulations. This is no coincidence. As demonstrated by Sanders and Erkal, gap creation is a dispersion phenomenon, where stars affected by a perturber acquire slightly different orbital frequencies, causing them to gradually spread apart over time. Thus, smaller gaps are indicative of more recent encounters.
      
    It is also expected that a recent flyby would appear in most simulations, as uncertainties in a cluster's precise location increase as we look further back in time. In general, after enough time passes, different orbital solutions \textit{phase mix}, meaning a body could be found anywhere in its phase space given its Hamiltonian and initial conditions. The characteristic phase-mixing time scale is proportional to its orbital period. However, since the encounter between NGC~2808 and Palomar~5's stream occurred less than one orbital period ago for either cluster, the orbital solutions remain relatively similar, causing the flyby to appear consistently in most simulations.      

    Next, we observe that most gaps occur in the leading tail. We believe this to be a coincidence, as we expect there to be no physical motivator to cause more gaps to occur in one tail than another. First, stars escape from the globular cluster's L1 and L2 Lagrange points at roughly equal rates, leading the same size and number of stars per tail. The two main differences each tail is that the particles find themselves at different locations on the orbital phase at a given point and time. The second is that the leading tail is about 400~pc closer to the Galactic center than the trailing tail. However, this difference is negligible compared to the orbital radius of 8-12kpc. As such, we expect the likelihood of a gap appearing in either tail to be equal, with the presence of one gap not influencing the probability of another. 


    Yet, the observation of only 8 gaps in the trailing tail out of 73 total gaps is inconsistent with this expectation. However, 44 of these gaps are associated with NGC~2808. As shown in Fig.\ref{fig:histogram_impact_time}, all of these gaps result from the same recent encounter. We thus discard NGC~2808 for the current analysis, as its gaps violate the assumption of independent events. After excluding these, we are left with 21 gaps in the leading tail and 8 in the trailing tail across the remaining simulations. By integrating the binomial distribution, we compute that the probability of having 8 successes or fewer given 29 trials is approximately 1.2\%. While this outcome is unlikely it is not impossible. Additionally, by studying Fig.~\ref{fig:histogram_impact_time}, we notice that, although in much smaller numbers compared to NGC~2808, other pertubers cause gaps at the same time accross different simulations. These could also be violating the assumption of independent events. 

    Overall, across 50 simulations, we observe 73 gaps. The distribution of simulations with a given number of gaps is as follows: 0 gaps in 4 simulations, 1 gap in 25 simulations, 2 gaps in 16 simulations, 3 gaps in 4 simulations, and 4 gaps in 1 simulation. Excluding NGC~2808, we are left with 29 gaps, leading to an expected range of 0.6-1.5 gaps per simulation.
...
\begin{appendix}
  ...
  \section{Gap Detection} \label{sec:gap_detection}
    ...
    \section{Perturber Identification} \label{sec:Perturber_Identification}
      ...
      Fig.~\ref{fig:force-on-orbit} contains some interesting information. Notice the periodic ribbions of force in the $(t,\tau)$ plane. This is due to pericenter passages where Palomar 5 is getting closer to the center of mass of the globular cluster system. Additionally, for the impacts of NGC104 and NGC7078, wakes can be observed in the density map. Another important aspect, is that the strongest peak in gravitational force does not necessarily create a gap. Notice how NGC5272, which was labeled with 0 to indicate that it has the greatest local maxima does not have a gap. This is manifold, for instance, the force needs to be modulated with the time of impact because the  important quantity that determins gap creation.  Additionally, there is an offset of about 200~pc between the stream at the orbit, as seen when viewing the stream in tail coordinates, so peaks upon the orbit are good proxy's for the stream but are not definitive. We found that the top five greatest impacts accounted for all gaps, except for Sampling~014 as shown in Fig.~\ref{fig:gallery3}, whose gap from NGC~6584 corresponded to the 7th peak. 
